...
def _format_job(job: Dict[str, Any]) -> Dict[str, Any]:
    ...
    if job.get("delivery_confirmation"):
        result["delivery_confirmation"] = job["delivery_confirmation"]
    return result
    ...
    try:
        ...
        if normalized == "update":
            ...
            if delivery_confirmation is not None:
                updates["delivery_confirmation"] = delivery_confirmation
    ...
    "parameters": {
        ...
        "properties": {
            ...
            "delivery_confirmation": {
                "type": "string",
                "enum": ["adapter_ack", "message_id"],
                "description": "Delivery safety level. adapter_ack accepts an explicit success response (default). message_id additionally requires a platform message ID; use it for delivery canaries and source-acknowledged critical alerts. A missing ID is quarantined as ambiguous and is never auto-resent."
            },
        },
        "required": ["action"]
    }
...
registry.register(
    ...
    handler=lambda args, **kw: (lambda _mo=_resolve_model_override(args.get("model")): cronjob(
        ...
        delivery_confirmation=args.get("delivery_confirmation"),
